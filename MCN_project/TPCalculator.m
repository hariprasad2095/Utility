function TotalTP = TPCalculator(BeamRSRQ)

error = 0;

snr = 1/((1/(12*BeamRSRQ))-(1/12));

SNRdB = 10*log10(abs(snr));% SNR in dB
...
SNRComp = [-6.9360 -5.1470 -3.1800 -1.2530 0.7610 2.6990 4.6940 6.5250 8.5730 10.3660 12.2890 14.1730 15.8880 17.8140 19.8290];

I = find(SNRComp > SNRdB);
...
ModScheme = [2 2 2 2 2 2 4 4 4 6 6 6 6 6 6];
CodeRate  = [78 120 193 308 449 602 378 490 616 466 567 666 772 873 948];
...
elseif (ModScheme(CQI) == 4)
    pdsch.Modulation = "16QAM";
elseif (ModScheme(CQI) == 6)
...
LocCodeRate = CodeRate(CQI);

% Coding rate
if pdsch.NumCodewords == 1
    codeRate = LocCodeRate/1024;
else
    codeRate = [LocCodeRate LocCodeRate]./1024;
end
...
TotalTp = (NumRB*NumREPerRB*NumSymbols*(1-error_rate)*ModScheme(CQI)*(LocCodeRate/1024)*totalNoSlots)*100;

%disp("Error Rate for"+ " "+x(+","+y(i)+","+z(i)+" is "+error_rate)
disp("Total TP: "+TotalTp)

TotalTP = TotalTp/(1024^2);
end
